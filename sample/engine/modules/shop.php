...
$FormArr = array(
"title"=>array("title"=>"Íàçâàíèå", "descr"=>"Íàçâàíèå ãîðîäà")
);

$CheckDataArr = array(
"text"=>array("type"=>"required", "msg"=>"Ïîëå 'Ñîîáùåíèå' îáÿçàòåëüíî ê çàïîëíåíèþ")
);
...
$CheckDataArr = array(
//"title"=>array("type"=>"required", "msg"=>"Ïîëå 'Íàçâàíèå' îáÿçàòåëüíî ê çàïîëíåíèþ"),
//"info"=>array("type"=>"required", "msg"=>"Ïîëå 'Îïèñàíèå' îáÿçàòåëüíî ê çàïîëíåíèþ"),
//"email"=>array("type"=>"email", "msg"=>"Ïîæàëóéñòà, çàïîëíèòå ïîëå Email ïðàâèëüíî")
);
...
$FormArr = array(
"title"=>array("title"=>"Íàçâàíèå", "descr"=>"Íàçâàíèå ñòðàíû")
);
...
$FormArr = array(
"title"=>array("title"=>"Íàçâàíèå", "descr"=>"Çàêàç")
);

$CheckDataArr = array(
"cart_id"=>array("type"=>"required", "msg"=>"Ïîëå 'ID êîðçèíû' îáÿçàòåëüíî ê çàïîëíåíèþ")
);
...
$MyModule->CreatePage("ShowAll", "Ãëàâíàÿ", $PageConf, $TagList, $UserRights, $UrlPath, "shop/main.tpl", $MetaArr);
// -------------------
if($_GET['op']=='category')
$MyModule->CreatePage("ShowAll", "Ãëàâíàÿ", $PageConf, $TagList, $UserRights, $UrlPath, "shop/main1.tpl", $MetaArr);
// when we set our data classes we need to make our pages to show
$PageConf = array(
"type"=>"static" // define type of the page - static ot form
);
...
$MetaArr = array(
"title" => $ShopConfig['title']['value'],
"keywords" => "äîñêà, îáúÿâëåíèÿ, êëþ÷, ñëîâî",
"description" => "Ëó÷øàÿ äîñêà îáúâëåíèé íàøåãî ãîðîäà!!!"
);

$MyModule->CreatePage("checkout", "Îôîðìëåíèå çàêàçà", $PageConf, $TagList, $UserRights, $UrlPath, "shop/checkout.tpl", $MetaArr);
// ---------------------->
...
$MetaArr = array(
"title" => "Êîíôèãóðàöèÿ :: ".$ShopConfig['title']['value'],
"keywords" => "äîñêà, îáúÿâëåíèÿ, êëþ÷, ñëîâî",
"description" => "Ëó÷øàÿ äîñêà îáúâëåíèé íàøåãî ãîðîäà!!!"
);

$MyModule->CreatePage("config", "Êîíôèãóðàöèÿ ìîäóëÿ", $PageConf, $TagList, $UserRights, $UrlPath, "shop/config.tpl", $MetaArr);
...
$MyModule->SetLangVar("ru", "checkbox_yes", "åñòü");
$MyModule->SetLangVar("ru", "checkbox_no", "íåò");
// Initiate languages
$MyModule->SetLangVar("ru", "categories", "êàòåãîðèè");
$MyModule->SetLangVar("ru", "cities", "ãîðîäà");
$MyModule->SetLangVar("ru", "products", "îáúÿâëåíèÿ");
$MyModule->SetLangVar("ru", "countries", "ñòðàíû");


$MyModule->SetLangVar("ru", "CfgAdmin", "Êîíôèãóðàöèÿ ìîäóëÿ");
$MyModule->SetLangVar("ru", "AddLink", "Äîáàâèòü");
$MyModule->SetLangVar("ru", "ShowList", "Ñïèñîê");
$MyModule->SetLangVar("ru", "Edit", "Èçìåíèòü");
$MyModule->SetLangVar("ru", "Delete", "Óäàëèòü");
$MyModule->SetLangVar("ru", "ModerList", "Ìîäåðàöèÿ");
$MyModule->SetLangVar("ru", "publ", "Îïóáëèêîâàòü");
$MyModule->SetLangVar("ru", "unpubl", "Ñíÿòü");

$MyModule->SetLangVar("ru", "msg_cities_add", "Ñïàñèáî, ãîðîä óñïåøíî äîáàâëåí");
$MyModule->SetLangVar("ru", "msg_categories_add", "Ñïàñèáî, êàòåãîðèÿ óñïåøíî äîáàâëåíà");
$MyModule->SetLangVar("ru", "msg_products_add", "Ñïàñèáî, îáúÿâëåíèå óñïåøíî äîáàâëåíî.");
$MyModule->SetLangVar("ru", "msg_countries_add", "Ñïàñèáî, ñòðàíà óñïåøíî äîáàâëåíî.");
$MyModule->SetLangVar("ru", "msg_comments_add", "Ñïàñèáî,  êîììåíòàðèé óñïåøíî äîáàâëåí.");

$MyModule->SetLangVar("ru", "msg_cities_edit", "Ñïàñèáî, ãîðîä óñïåøíî èçìåíåí");
$MyModule->SetLangVar("ru", "msg_categories_edit", "Ñïàñèáî, êàòåãîðèÿ óñïåøíî èçìåíåíà");
$MyModule->SetLangVar("ru", "msg_products_edit", "Ñïàñèáî, îáúÿâëåíèå óñïåøíî èçìåíåíî.");
$MyModule->SetLangVar("ru", "msg_countries_edit", "Ñïàñèáî, ñòðàíà óñïåøíî èçìåíåíà");
$MyModule->SetLangVar("ru", "msg_comments_edit", "Ñïàñèáî, êîììåíòàðèé óñïåøíî èçìåíåí");


$MyModule->SetLangVar("ru", "msg_cities_del", "Ñïàñèáî, ãîðîä óñïåøíî óäàëåí");
$MyModule->SetLangVar("ru", "msg_categories_del", "Ñïàñèáî, êàòåãîðèÿ óñïåøíî óäàëåíà");
$MyModule->SetLangVar("ru", "msg_products_del", "Ñïàñèáî, îáúÿâëåíèå óñïåøíî óäàëåíî.");
$MyModule->SetLangVar("ru", "msg_countries_del", "Ñïàñèáî, ñòðàíà óñïåøíî óäàëåíà.");
$MyModule->SetLangVar("ru", "msg_comments_del", "Ñïàñèáî, êîììåíòàðèé óñïåøíî óäàëåí");

$MyModule->SetLangVar("ru", "msg_record_exists", "Òî÷íî òàêàÿ æå çàïèñü óæå ñóùåñòâóåò â áàçå.");
$MyModule->SetLangVar("ru", "msg_record_none", "Òîâàðû â äàííîé êàòåãîðèè âðåìåííî îòñóòñòâóåò.");

$MyModule->SetLangVar("ru", "msg_restricted", "Ó âàñ íåäîñòàòî÷íî ïðàâ äëÿ ïðîñìîòðà ýòîé ñòðàíèöû. <br><a href=\"javascript:history.go(-1)\">Íàçàä</a>");
...
$MyModule->SetLangVar("ru", "addimgfield", "Äîáàâèòü ïîëå");
...
class shopPlugin
{
	...
	function GetAddLink()
	{
		...
		if($this->VarMod->MyConfig['allow_add']['value'])
		{
			...
			{
				return "<a style=\"font-size: 14px;\" href=\"".$this->VarMod->GetLink("products", "additem")."\">Äîáàâèòü îáúÿâëåíèå</a>";
			}
			else
			{
				$Restr = "Äîáàâèòü îáúÿâëåíèå (âàì íå õâàòàåò ïðàâ)";
				if(!$is_logged)
				{
					$Restr .= " <a style=\"font-size: 14px;\" href=\"/index.php?do=register\">Ðåãèñòðàöèÿ</a>";
				}
				return $Restr;
			}
		}
		 ...
		 function translitIt($str) 
{
	...
	function MakeItemList($Params)
	{
		...
		<input type="submit" class="buybut" value="Â êîðçèíó" />
		</form>
		';
	...
	function ShowFull($ItemDataArr)
	{
		...
		if($_GET['class']=="carts")
		{
			...
			else
			{
				...
				{
					$metatags['title'] =  "Âàøà êîðçèíà :: ".$this->VarMod->MyConfig['title']['value'];
					// Now make items list
					...
					if(count($CartContents)>0)
					{
						foreach($CartContents as $key=>$utin)
						{
							...
							$ItemDataArr['items'] .= "".$this->VarMod->GetTrueValue($MyProd['category'], $this->VarMod->MyClasses['products']['db']['category'])." - ".$MyProd['title']." ".$Qty."x - ".$FullPrice." ðóá - "."<form style=\"display: inline;\" action=\"".$BuyLink."\" method=\"post\"><input type=\"text\" name=\"newqty\" style=\"width: 40px;\" value=\"".$Qty."\" /><input type=\"submit\" value=\"Èçìåíèòü\" /></form> <a href=\"".$this->VarMod->GetLink("carts", "deleteitem", array("id"=>$ItemDataArr['id'], "did"=>$key))."\">Óäàëèòü</a>"."<br />";
						}
					}
					if(count($CartContents)>0)
					{
					...
					{
						$ItemDataArr['items'] = "Ïîêà íåò òîâàðîâ â êîðçèíå";
						$ItemDataArr['formlnk'] = '';
	...
	{
	//	<span class="express">Ñðî÷íî!</span>
		if($this->VarMod->UserCheckRights('admin'))
			{
		$TplText = str_replace("{plg_upped}", " | <a href=\"/index.php?do=shop&class=products&op=itemup&id=".$rowData['id']."\">Ïîäíÿòü</a>", $TplText);
		}
		else
		{
		...
		if($rowData['urgent']=="1")
		{
			$TplText = str_replace("{plg_srochno}", "<span class=\"srochbut\"><b>ÑÐÎ×ÍÎ</b></span>", $TplText);
		}
		else
		{
	...
	function CheckOutSend()
	{
		...
		{
			$MesData = "Ïîëÿ Email, ÔÈÎ è Òåëåôîí îáÿçàòåëüíû äëÿ çàïîëíåíèÿ <br /><a href=\"\">Íàçàä</a>";
		...
		$this->SendMessage($this->VarMod->MyConfig['site_mail']['value'], $StrEmail, "Íîâûé çàêàç â ".$this->VarMod->MyConfig['title']['value'], $MessArr, "mailadmin");
	    $this->SendMessage($StrEmail, $this->VarMod->MyConfig['site_mail']['value'], "Âàø çàêàç â ".$this->VarMod->MyConfig['title']['value'], $MessArr, "mailuser");
	...
	function WebMoneyForm($MessArr)
	{
		...
		$FormStr = "";
		$FormStr .= '<h4>Àêòèâèðóéòå WebMoney Keeper ïåðåä ïåðåõîäîì ê ñòðàíèöå ïëàòåæà.</h4>'; // Please remember to activate WebMoney Keeper prior to processing next step
		$FormStr .= '<form action="'.$url.'" method="post">';
		$FormStr .= '<input type="submit" value="Îïëàòèòü çàêàç" name="formSubmit" class="button"/>';
	...
	function PlatronForm($MessArr)
	{
		...
		$form_fields = array(
			...
			'cms_payment_module'=> 'DLE_SHOP',
			'pg_salt'			=> rand(21,43433), // Ïàðàìåòðû áåçîïàñíîñòè ñîîáùåíèÿ. Íåîáõîäèìà ãåíåðàöèÿ pg_salt è ïîäïèñè ñîîáùåíèÿ.
		);
			...
			if($MyCartId==$ItemDataArr['user_id'])
			{
					...
					if(count($CartContents)>0)
					{
						...
						foreach($CartContents as $key=>$utin)
						{
							...
							$ItemDataArr['items'] .= "<b>".$cnt.".</b> <a href=\"{$BaseLink}".$this->VarMod->GetLink("products", "itemshow", array("id"=>$utin['prod']))."\">".$this->VarMod->GetTrueValue($MyProd['category'], $this->VarMod->MyClasses['products']['db']['category'])." - ".$MyProd['title']."</a> ".$Qty."x - ".$FullPrice." ðóá. <br />";
							$cnt++;
						}
					}
	...
	function ShowCart()
	{
		...
		{
			$tpl->copy_template = "<div align=\"center\"><b>Êîðçèíà ïóñòà</b></div>";
			$tpl->compile('mycart');
			$tpl->clear();
		}
	}
	...
	function ShowCats()
	{
		...
		if($RetStr=="")
		{
			$RetStr = "Â äàííîé êàòåãîðèè íåò ïîäêàòåãîðèé";
		}
		return $RetStr;
	}
